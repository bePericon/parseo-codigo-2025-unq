...
Para lograr agregar esta funcionalidad de Eiffel al interprete, basto con lograr que el \textbf{lexer.l} identifique
los comentarios y no genere ningún token. Ademas se borro del \textbf{parser.y} el token ya que no iba a ser consumido.
...
Antes de los cambios el analizador léxico estaba obteniendo bien al operando '+', pero solo como la operación
aritmética de suma, no funcionando correctamente cuando los operandos son de tipo \textit{STRING}.
Para lograr que ademas de la operación aritmética logre la concatenación de strings se realizo un cambio sobre
el interprete. Cuando la función \textbf{evaluate\_ast} identifica el nodo \textbf{N\_BINOP} en el AST para la
operación/signo '+', se decide si realizar la suma o concatenación dependiendo del tipo de cada operando. 
En caso de no tener tipos admisibles se lanza un error.
...
\subsection{Multiples lineas de declaración de variables}
\vspace{2mm}
...
\vspace{2mm}
Se agregó en el \textbf{lexer.l} la obtención del token para números negativos.
...
{NEG_NUMBER}   {yylval.str = strdup(yytext); yycolumn += yyleng; return NEG_NUMBER; }
...
\newpage
\noindent
Y también su lectura en el \textbf{parser.y} para crear el nodo \textbf{INTEGER} correspondiente,
pero con un valor negativo.
...
\subsection{Conclusión de re-entrega}
\vspace{2mm}

Con pequeños cambios y agregado de funcionalidades no tan complejas, el interprete se ha vuelto un poco más robusto
frente a distintas pruebas. \\

\noindent
Se agregan nuevos test: \\
\hspace{4mm}\textbf{11\_comments.e}, \textbf{12\_concat\_strings.e}, \textbf{13\_multi\_decl\_vars.e}, \textbf{14\_negative\_numbers.e}
