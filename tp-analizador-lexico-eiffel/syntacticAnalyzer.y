...
/* --- tokens --- */
%token <str> CLASS CREATE FEATURE DO END IF THEN ELSE FROM UNTIL LOOP INHERIT MAIN MAKE PRINT REQUIRE INVARIANT ENSURE
%token <str> IDENTIFIER RESERVED_TYPE
%token <str> NUMBER STRING TRUE FALSE REAL
...
token_line:
    ...
    | REQUIRE                       { printf("TOKEN_PRINT                   -> %s\n", $1); free($1); }
    | ENSURE                        { printf("TOKEN_PRINT                   -> %s\n", $1); free($1); }
    | INVARIANT                     { printf("TOKEN_PRINT                   -> %s\n", $1); free($1); }

    | IDENTIFIER                    { printf("TOKEN_IDENTIFIER              -> %s\n", $1); free($1); }
    | NUMBER                        { printf("TOKEN_NUMBER                  -> %s\n", $1); free($1); }
    | REAL                          { printf("TOKEN_REAL                    -> %s\n", $1); free($1); }
